...
class RevUI:
    ...
    can_mode_map = {v: k for k, v in ControlType.__members__.items()}

    def update_tk_widgets(self, sim):

        for k, data in hal_data["CAN"].items():

            if data["type"] not in ["sparkmax"]:
                continue
            ...
            enc_txt.set("E: %s" % round(data["position"]))

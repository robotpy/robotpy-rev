...
def test_position_pid_telemetry_and_tuning(robot, control):
    ...
    with control.run_robot():
        ...
        nt.get_entry("/Tunables/Set Rotations").set_double(12.5)
        control.step_timing(seconds=0.2, autonomous=False, enabled=True)
        assert robot.p_gain == 0.25
        assert robot.d_gain == 0.125
        assert nt.get_entry("/Telemetry/SetPoint").get_double(-1) == 12.5
        ...
        assert robot.pid_controller.get_setpoint().get() == 12.5
        assert (
            robot.pid_controller.get_control_type()
            == rev.SparkBase.ControlType.POSITION
        )
